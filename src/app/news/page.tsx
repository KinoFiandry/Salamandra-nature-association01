        ...
        {loading ? (
          ...
          <div className="space-y-16">
            {news.map((item, i) => (
              ...
              >
                <div className={`overflow-hidden rounded-[3rem] h-[400px] border border-emerald-100 ${i % 2 === 1 ? 'md:order-last' : ''}`}>
                  <img
                ...
                <div>
                  <div className="flex items-center gap-3 text-emerald-600 mb-6 font-bold">
                    <Calendar className="w-5 h-5" />
                    <span>
                      {format(new Date(item.created_at), 'PPP', { locale: language === 'fr' ? fr : enUS })}
                    </span>
                  </div>
                  <h2 className="text-4xl font-bold text-emerald-900 mb-6 group-hover:text-emerald-600 transition-colors">
                    {language === 'en' ? item.title_en : item.title_fr}
                  </h2>
                  <p className="text-emerald-800/70 text-xl leading-relaxed mb-10">
                    {language === 'en' ? item.content_en : item.content_fr}
                  </p>
                  <button className="flex items-center gap-3 text-emerald-600 font-black text-lg group/btn hover:translate-x-2 transition-transform">
